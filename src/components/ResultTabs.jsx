...
function computeRanks(ranking, mode) {
  const score = (u) => {
    if (mode === 'post') return u.postCount
    if (mode === 'comment') return u.commentCount
    return u.postCount + u.commentCount
  }
  const ranks = []
  let rank = 1
  for (let i = 0; i < ranking.length; i++) {
    if (i > 0 && score(ranking[i]) < score(ranking[i - 1])) rank = i + 1
    ranks.push(rank)
  }
  return ranks
}
...
export default function ResultTabs({ result }) {
  ...
  const ranks = computeRanks(ranking, activeTab)
          ...
          <tbody>
            {ranking.map((u, i) => (
              <tr key={i} className={i < 3 ? `rank-top-${i + 1}` : ''}>
                <td className="td-rank">{ranks[i]}위</td>
...
function buildHtml(result, mode, ranking) {
  ...
  const ranks = computeRanks(ranking, mode)
...
function buildText(result, mode, ranking) {
  ...
  if (mode === 'combined') {
    ...
    ranking.forEach((u, i) => {
      const id = u.isFluid ? u.ip : u.uid
      lines.push(`${ranks[i]}위\t${u.nick}\t${id}\t${u.postCount}\t${u.commentCount}\t${u.postCount + u.commentCount}`)
    })
  } else if (mode === 'post') {
    lines.push('순위\t\t닉네임\t\tID/IP\t\t글 수')
    ranking.forEach((u, i) => {
      const id = u.isFluid ? u.ip : u.uid
      lines.push(`${ranks[i]}위\t${u.nick}\t${id}\t${u.postCount}`)
    })
  } else {
    lines.push('순위\t\t닉네임\t\tID/IP\t\t댓글 수')
    ranking.forEach((u, i) => {
      const id = u.isFluid ? u.ip : u.uid
      lines.push(`${ranks[i]}위\t${u.nick}\t${id}\t${u.commentCount}`)
    })
  }
